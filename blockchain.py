...
'''
Create the edits for the file, where user will enter the edit at any node, 
before the edit is send to the node containing the file data, 
a temporary fat log will be kept, 
every other node will get the fat log (each fat will be in the following format [‘file_name, ‘node_port’, ‘node_address’] 
